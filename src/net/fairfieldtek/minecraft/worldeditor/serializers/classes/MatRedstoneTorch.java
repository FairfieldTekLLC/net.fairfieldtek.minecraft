 ...
 * @author geev
 */
public class MatRedstoneTorch extends Serializer implements ISerializer {
    public static boolean isType(Block block) {
        return block.getState().getData() instanceof org.bukkit.material.RedstoneTorch;
    }

    private int FacingIdx;
    public MatRedstoneTorch(Block block, int xOffset, int yOffset, int zOffset, net.fairfieldtek.minecraft.worldeditor.container.SchematicDef schematicDef) {
        super(block, xOffset, yOffset, zOffset, schematicDef);
        if (!isType(block)) {
            throw new RuntimeException("Invalid Type");
        }

        org.bukkit.material.RedstoneTorch t = (org.bukkit.material.RedstoneTorch) block.getState().getData();
