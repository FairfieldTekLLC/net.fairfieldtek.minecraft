 ...
 * @author geev
 */
public class MatSkull  extends Serializer implements ISerializer {
    public static boolean isType(Block block) {
        return block.getState().getData() instanceof org.bukkit.material.Skull;
    }
    private int FacingIdx;    
    public MatSkull(Block block, int xOffset, int yOffset, int zOffset, net.fairfieldtek.minecraft.worldeditor.container.SchematicDef schematicDef) {
        super(block, xOffset, yOffset, zOffset, schematicDef);
        if (!isType(block)) {
            throw new RuntimeException("Invalid Type");
        }
        
        org.bukkit.material.Skull t = ( org.bukkit.material.Skull)block.getState().getData();
