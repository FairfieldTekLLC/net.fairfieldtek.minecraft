 ...
 * @author geev
 */
public class MatLadder extends Serializer implements ISerializer {
    public static boolean isType(Block block) {
        return block.getState().getData() instanceof org.bukkit.material.Ladder;
    }

    private int FacingIdx;
    public MatLadder(Block block, int xOffset, int yOffset, int zOffset, net.fairfieldtek.minecraft.worldeditor.container.SchematicDef schematicDef) {
        super(block, xOffset, yOffset, zOffset, schematicDef);
        if (!isType(block)) {
            throw new RuntimeException("Invalid Type");
        }

        org.bukkit.material.Ladder d = (org.bukkit.material.Ladder) block.getState().getData();
