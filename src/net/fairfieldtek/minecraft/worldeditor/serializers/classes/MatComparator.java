 ...
 * @author geev
 */
public class MatComparator extends Serializer implements ISerializer {
    public static boolean isType(Block block) {
        return (block.getState().getData() instanceof org.bukkit.material.Comparator);
    }
    private int FacingIdx;
    public MatComparator(Block block, int xOffset, int yOffset, int zOffset, net.fairfieldtek.minecraft.worldeditor.container.SchematicDef schematicDef) {
        super(block, xOffset, yOffset, zOffset, schematicDef);
        if (!isType(block)) {
            throw new RuntimeException("Invalid Type");
        }
        org.bukkit.material.Comparator comp = (org.bukkit.material.Comparator) block.getState().getData();
        org.bukkit.block.Comparator comp1 = (org.bukkit.block.Comparator) block.getState();
        
        setFacing(comp.getFacing());
    }
