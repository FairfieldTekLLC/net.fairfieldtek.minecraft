 ...
 * @author geev
 */
public class MatDispenser extends Serializer implements ISerializer {
    public static boolean isType(Block block) {
        return block.getState().getData() instanceof org.bukkit.material.Dispenser;
    }

    private int FacingIdx;
    public MatDispenser(Block block, int xOffset, int yOffset, int zOffset, net.fairfieldtek.minecraft.worldeditor.container.SchematicDef schematicDef) {
        super(block, xOffset, yOffset, zOffset, schematicDef);
        if (!isType(block)) {
            throw new RuntimeException("Invalid Type");
        }
        
        org.bukkit.material.Dispenser d = (org.bukkit.material.Dispenser)block.getState().getData();
        org.bukkit.block.Dispenser dd = (org.bukkit.block.Dispenser)block.getState();
