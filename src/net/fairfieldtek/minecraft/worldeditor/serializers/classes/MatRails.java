 ...
 * @author geev
 */
public class MatRails  extends Serializer implements ISerializer {
    public static boolean isType(Block block) {
        return block.getState().getData() instanceof org.bukkit.material.Rails;
    }
     private int FacingIdx;
     private int IsOnSlope;
     public MatRails(Block block, int xOffset, int yOffset, int zOffset, net.fairfieldtek.minecraft.worldeditor.container.SchematicDef schematicDef) {
         super(block, xOffset, yOffset, zOffset, schematicDef);
         if (!isType(block)) {
             throw new RuntimeException("Invalid Type");
         }
         
         org.bukkit.material.Rails rail = (org.bukkit.material.Rails)block.getState().getData();
         setFacing(rail.getDirection());
         setIsOnSlope(rail.isOnSlope());
    ...
    public final  boolean getIsOnSlope(){
        return IsOnSlope == 1;
    }
    public final  void setIsOnSlope(boolean flag){
        if (flag)
            IsOnSlope=1;
        else
            IsOnSlope=0;
