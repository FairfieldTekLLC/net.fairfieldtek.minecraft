 ...
 * @author geev
 */
public class MatBed extends Serializer implements ISerializer {
    public static boolean isType(Block block) {
        return (block.getState().getData() instanceof  org.bukkit.material.Bed);
    }

    private int DyeColor;
    private int FacingIdx;
    public MatBed(Block block, int xOffset, int yOffset, int zOffset, net.fairfieldtek.minecraft.worldeditor.container.SchematicDef schematicDef) {
        super(block, xOffset, yOffset, zOffset, schematicDef);
        if (!isType(block)) {
            throw new RuntimeException("Invalid Type");
        }
        
        org.bukkit.material.Bed mBed = (org.bukkit.material.Bed) block.getState().getData();
        
        if (!mBed.isHeadOfBed()) {
            this.setMaterialType(Material.AIR);
            return;
        }
        setFacing(mBed.getFacing());
        setColor(((org.bukkit.block.Bed) block.getState()).getColor());
